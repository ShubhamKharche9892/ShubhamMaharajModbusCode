...
void ExecuteCommand(char commandStr[])
{
    ...
    else if (strcmp(commandStr, "getval") == 0)
    {
        ...
        if (GetTempHumValue())
        {
            ...
            SerialBT.println("request successfull\n");
        }
    }
    ...
    else if (strcmp(commandStr, "setalarm") == 0)
    {
        ...
        Serial.print("\nsending Modbus request...\n");
        SerialBT.print("\nsending Modbus request...\n");
        SerialBT.print("ON/OFF: ");
        delay(2000);
        String e = SerialBT.readStringUntil('\n');
        if (e == "0")
        {
            f = 0;
        }
...
void ModbusInit(void)
{
    ...
    //SerialBT.printf("id: 0x%02x fc: 0x%02x len %u: 0x", serverAddress, fc, length);
    for (size_t i = 0; i < length; ++i) {
      Serial.printf("%02x", data[i]);
      //SerialBT.printf("%02x", data[i]);
    }
    printf("\n");
        ...
        SerialBT.printf("Humidity: %7.1f\n", h);
        SerialBT.printf("Temperature: %7.1f\n", t);
        }
        ...
        SerialBT.printf("Temperature Lower Limit: %7.1f\n", t);
        ...
        SerialBT.printf("Temperature Upper Limit: %7.1f\n", t);
        }
        else if (address == 0x001C){
        uint16_t temp = convertFrom8To16(data[0], data[1]);
        float t=temp/10.0;
        printf("Humidity Lower Limit: %7.1f\n", t);
        SerialBT.printf("Humidity Lower Limit: %7.1f\n", t);
        }
        else if (address == 0x001B){
        uint16_t temp = convertFrom8To16(data[0], data[1]);
        float t=temp/10.0;
        printf("Humidity Upper Limit: %7.1f\n", t);
        SerialBT.printf("Humidity Upper Limit: %7.1f\n", t);
        }//0x001F
...
if(fc==0x06){
     ...
     SerialBT.printf("id: 0x%02x fc: 0x%02x ", serverAddress, fc);
     SerialBT.printf("\nwrote value successfully....");
